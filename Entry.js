if (self.CavalryLogger) {
	CavalryLogger.start_js(["ZZIjT"]);
}

__d("FormTypeABTester", ["Base64", "Event"], (function (a, b, c, d, e, f) {
		e.exports = a;
		function a(a) {
			var c = 32,
			d = 65,
			...
			for (var v = 0; v < 10; v++)
				r.push(0), s.push(0);
			for (var v = 0; v < 10; v++)
				s.push(0), t.push(0), u.push(0);
			v = function (a) {
				var b = window.event ? Date.now() : a.timeStamp;
				a = window.event ? window.event.keyCode : a.which;
				a %= 128;
				a >= d && a <= e || a == c ? l++ : a >= f && a <= g ? m++ : a >= h && a <= i || a >= j && a <= k ? n++ : o++;
				p[a] = b;
				if (q != null && q !== 0) {
					var r = b - q;
					r >= 0 && (a >= d && a <= e || a == c) && (r < 400 ? s[Math.floor(r / 20)]++ : r < 1e3 ? t[Math.floor((r - 400) / 60)]++ : r < 3e3 && u[Math.floor((r - 1e3) / 200)]++)
				}
				q = b
			};
			var w = function (a) {
			...
			this.getDataVect = function () {
				var a = s.concat(t, u);
				return x(a).concat(x(r), [l / 2, m / 2, n / 2, o / 2])
			};
			this.getData = function () {
				return b("Base64").encodeNums(this.getDataVect())
			};
			b("Event").listen(a, {
				keyup: w.bind(this),
				keydown: v.bind(this)
		...
		function h() {
			...
			function e(a, b, c) {
				...
				for (var f = 0; f < b.length; f += 2) {
					if (c === 32)
						e += d(b[f]).toUpperCase(), e += " ", e += d(b[f + 1]).toUpperCase();
					else if (c === 64)
						e += d(b[f + 1]).toUpperCase(), e += d(b[f]).toUpperCase();
					else
		...
		function l() {
			...
			function m(a, b) {
				...
				for (c = 0; c < 32; c++)
					l[c] = d(a.b, 4 * c);
				for (c = 0; c < 12; c++)
					e(0, 8, 16, 24, h[c * 16 + 0], h[c * 16 + 1]), e(2, 10, 18, 26, h[c * 16 + 2], h[c * 16 + 3]), e(4, 12, 20, 28, h[c * 16 + 4], h[c * 16 + 5]), e(6, 14, 22, 30, h[c * 16 + 6], h[c * 16 + 7]), e(0, 10, 20, 30, h[c * 16 + 8], h[c * 16 + 9]), e(2, 12, 22, 24, h[c * 16 + 10], h[c * 16 + 11]), e(4, 14, 16, 26, h[c * 16 + 12], h[c * 16 + 13]), e(6, 8, 18, 28, h[c * 16 + 14], h[c * 16 + 15]);
				for (c = 0; c < 16; c++)
		...
		function p() {
			...
			function i(a, d) {
				...
				for (i = 0; i < 16; i++)
					h[i] = b(a.b, 4 * i);
				for (i = 0; i < 10; i++)
					c(0, 4, 8, 12, h[f[i * 16 + 0]], h[f[i * 16 + 1]]), c(1, 5, 9, 13, h[f[i * 16 + 2]], h[f[i * 16 + 3]]), c(2, 6, 10, 14, h[f[i * 16 + 4]], h[f[i * 16 + 5]]), c(3, 7, 11, 15, h[f[i * 16 + 6]], h[f[i * 16 + 7]]), c(0, 5, 10, 15, h[f[i * 16 + 8]], h[f[i * 16 + 9]]), c(1, 6, 11, 12, h[f[i * 16 + 10]], h[f[i * 16 + 11]]), c(2, 7, 8, 13, h[f[i * 16 + 12]], h[f[i * 16 + 13]]), c(3, 4, 9, 14, h[f[i * 16 + 14]], h[f[i * 16 + 15]]);
				for (i = 0; i < 8; i++)
					a.h[i] ^= g[i] ^ g[i + 8]
			}
			function k(a, b) {
			...
			function l(a, b) {
				for (var c = 0; c < b.length; c++)
					a.c === 64 && (a.t += a.c, i(a, !1), a.c = 0), a.b[a.c++] = b[c]
			}
			function m(a) {
				a.t += a.c;
				while (a.c < 64)
		...
		function h() {
			(function (a) {
				...
				A.prototype.finish = function (a, b) {
					...
					this.h[0] = c & 65535;
					for (e = 1; e < 8; e++)
						c = (this.h[e] + this.pad[e] | 0) + (c >>> 16) | 0, this.h[e] = c & 65535;
				...
				function G(a) {
					...
					d = 1;
					for (b = 0; b < 16; b++)
						c = a[b] + d + 65535, d = Math.floor(c / 65536), a[b] = c - d * 65536;
					a[0] += d - 1 + 37 * (d - 1)
				}
				function H(a, b, c) {
					var d;
				...
				function S(a, c, d) {
					...
					L(f, d);
					for (g = 0; g < 16; g++)
						j[g] = f[g], l[g] = i[g] = k[g] = 0;
					i[0] = l[0] = 1;
					for (g = 254; g >= 0; --g)
						c = e[g >>> 3] >>> (g & 7) & 1, H(i, j, c), H(k, l, c), M(m, i, k), N(i, i, k), M(k, j, l), N(j, j, l), P(l, m), P(n, i), O(i, k, i), O(k, j, m), M(m, i, k), N(i, i, k), P(j, i), N(k, l, n), O(i, k, h), M(i, i, l), O(k, k, i), O(i, l, n), O(l, j, f), P(j, m), H(i, j, c), H(k, l, c);
					for (g = 0; g < 16; g++)
						f[g + 16] = i[g], f[g + 32] = k[g], f[g + 48] = j[g], f[g + 64] = l[g];
					d = f.subarray(32);
					c = f.subarray(16);
					Q(d, d);
					O(c, c, d);
				...
				function da(a, b, c, d) {
					...
					while (d >= 128) {
						for (y = 0; y < 16; y++)
							z = 8 * y + W, e[y] = c[z + 0] << 24 | c[z + 1] << 16 | c[z + 2] << 8 | c[z + 3], f[y] = c[z + 4] << 24 | c[z + 5] << 16 | c[z + 6] << 8 | c[z + 7];
						for (y = 0; y < 80; y++) {
							g = G;
							h = H;
							i = I;
							...
							if (y % 16 === 15)
								for (z = 0; z < 16; z++)
									A = e[z], B = f[z], C = B & 65535, D = B >>> 16, E = A & 65535, F = A >>> 16, A = e[(z + 9) % 16], B = f[(z + 9) % 16], C += B & 65535, D += B >>> 16, E += A & 65535, F += A >>> 16, w = e[(z + 1) % 16], x = f[(z + 1) % 16], A = (w >>> 1 | x << 32 - 1) ^ (w >>> 8 | x << 32 - 8) ^ w >>> 7, B = (x >>> 1 | w << 32 - 1) ^ (x >>> 8 | w << 32 - 8) ^ (x >>> 7 | w << 32 - 7), C += B & 65535, D += B >>> 16, E += A & 65535, F += A >>> 16, w = e[(z + 14) % 16], x = f[(z + 14) % 16], A = (w >>> 19 | x << 32 - 19) ^ (x >>> 61 - 32 | w << 32 - (61 - 32)) ^ w >>> 6, B = (x >>> 19 | w << 32 - 19) ^ (w >>> 61 - 32 | x << 32 - (61 - 32)) ^ (x >>> 6 | w << 32 - 6), C += B & 65535, D += B >>> 16, E += A & 65535, F += A >>> 16, D += C >>> 16, E += D >>> 16, F += E >>> 16, e[z] = E & 65535 | F << 16, f[z] = C & 65535 | D << 16
						}
						A = G;
						B = O;
						C = B & 65535;
				...
				function ha(a, b, c) {
					...
					F(a[3], f);
					for (e = 255; e >= 0; --e)
						d = c[e / 8 | 0] >> (e & 7) & 1, fa(a, b, d), ea(b, a), ea(a, a), fa(a, b, d)
				}
				function ia(a, c) {
					var d = [b(), b(), b(), b()];
					F(d[0], k);
				...
				function la(a, b) {
					...
					for (d = 63; d >= 32; --d) {
						...
						for (e = d - 32, f = d - 12; e < f; ++e)
							b[e] += c - 16 * b[d] * ka[e - (d - 32)], c = b[e] + 128 >> 8, b[e] -= c * 256;
						b[e] += c;
						b[d] = 0
					}
					c = 0;
		...
		function l() {
			(function (a, b) {
				if (typeof i === "object" && typeof j === "object")
					j.exports = b();
				else if (typeof k === "function" && k.amd)
					k([], b);
				else {
					b = b();
					for (var c in b)
						(typeof i === "object" ? i : a)[c] = b[c]
				}
			...
			return b("regeneratorRuntime").async(function (y) {
				while (1)
					...
					case 6:
						...
						x = g.subtle.generateKey(v, !0, ["encrypt", "decrypt"]).then(function (a) {
								var c = g.subtle.exportKey("raw", a);
								a = g.subtle.encrypt(w, a, d.buffer);
...
__d("tweetnacl-util-0.15.0", [], (function (a, b, c, d, e, f) {
		...
		function h() {
			(function (a, b) {
				...
				var a = {};
				function b(a) {
					if (!/^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$/.test(a))
...
__d("LoginFormController", ["AsyncRequest", "Button", "Cookie", "DeferredCookie", "DOM", "Event", "FBLogger", "Form", "FormTypeABTester", "WebStorage", "bx", "ge", "goURI", "guid", "promiseDone", "FBBrowserPasswordEncryption", "LoginServicePasswordEncryptDecryptEventTypedLogger"], (function (a, b, c, d, e, f) {
		var g;
		a = {
			init: function (a, c, d, e, f) {
				var g = this;
				this._initShared(a, c, d, e, f);
				this.isCredsManagerEnabled = !1;
				!f || !f.pubKey ? b("Event").listen(a, "submit", this._sendLoginShared.bind(this)) : b("Event").listen(a, "submit", function (b) {
					b.preventDefault(),
					g._sendLoginShared.bind(g)(),
					g._encryptBeforeSending(function () {
						a.submit()
					})
				})
			},
			initAsync: function (a, c, d, e, f) {
				var g = this;
				this._initShared(a, c, d, e, f);
				this.isCredsManagerEnabled = !0;
				this.emailInput = b("DOM").scry(a, 'input[id="email"]')[0];
				this.passwordInput = b("DOM").scry(a, 'input[id="pass"]')[0];
				this.errorBox = b("DOM").scry(a, 'input[id="error_box"]')[0];
				b("Event").listen(a, "submit", function (a) {
					a.preventDefault(),
					g._sendLoginRequest()
				});
				this._initSmartLockAccountChooser()
			},
			_initShared: function (a, c, d, e, f) {
				this.loginForm = a;
				this.loginButton = c;
				this.abTesting = e;
				this.loginFormParams = f;
				this.abTesting && (this.formABTest = new(b("FormTypeABTester"))(a));
				c = b("ge")("lgnjs");
				e = Math.floor(Date.now() / 1e3);
				c && (c.value = e);
				var h = (g || (g = b("WebStorage"))).getSessionStorage();
				f = h != null ? parseInt(h.getItem("LoginPollRateLimit"), 10) : 0;
				a = d != null;
				f > e - 60 && (a = !1);
				if (a) {
					var i;
					c = function () {
						b("Cookie").get("c_user") != null && (window.clearInterval(i), h != null && h.setItem("LoginPollRateLimit", Math.floor(Date.now() / 1e3).toString()), b("goURI")(d))
					};
					i = window.setInterval(c, 1e3);
					c()
				}
			},
			...
			_sendLoginShared: function () {
				var a = this;
				this.abTesting && this.loginForm.ab_test_data && (this.loginForm.ab_test_data.value = this.formABTest.getData());
				var c = b("guid")();
				this.loginForm.guid && (this.loginForm.guid.value = c);
				window.__cookieReload && window.clearInterval(window.__cookieReload);
				try {
					b("Button").setEnabled(this.loginButton, !1)
				} catch (a) {
				...
				window.setTimeout(function () {
					(function () {
						try {
							b("Button").setEnabled(a.loginButton, !0)
						} catch (b) {
							a.loginButton.disabled = !1
						}
					})
				}, 15e3);
				b("DeferredCookie").flushAllCookies()
			},
			_sendLoginRequest: function () {
				var a = this;
				this._sendLoginShared();
				if (this.login_form_params && this.login_form_params.pubKey)
					this._encryptBeforeSending(function () {
						var c = b("Form").serialize(a.loginForm);
						new(b("AsyncRequest"))().setURI(a.loginForm).setData(c).setHandler(a._onLoginResponse.bind(a)).send()
					});
				else {
					var c = b("Form").serialize(this.loginForm);
					new(b("AsyncRequest"))().setURI(this.loginForm.action).setData(c).setHandler(this._onLoginResponse.bind(this)).send()
				}
			},
			_onLoginResponse: function (a) {
				...
				if (a === null || a.error === null)
					return;
				b("DOM").replace(this.errorBox, a.error);
				b("Button").setEnabled(this.loginButton, !0)
			},
			redirect: function (a) {
				if (this.isCredsManagerEnabled) {
					var c = b("bx").getURL(b("bx")("875231"));
					c = new window.PasswordCredential({
							id: this.emailInput.value,
							password: this.passwordInput.value,
							iconURL: c
						});
					navigator.credentials && navigator.credentials.store(c).then(function () {
						window.setTimeout(function () {
			...
			_initSmartLockAccountChooser: function (a) {
				var c = this;
				a === void 0 && (a = "silent");
				window.PasswordCredential && (navigator.credentials !== null && navigator.credentials.get({
						password: !0,
						mediation: a
					}).then(function (b) {
						b !== null && b.type === "password" && b.password !== null && b.id !== null ? (c.emailInput.setAttribute("value", b.id), c.passwordInput.setAttribute("value", b.password), a === "required" && c._sendLoginRequest()) : (c.passwordInput.setAttribute("value", ""), a === "silent" && c._initSmartLockAccountChooser("required"))
					})["catch"](function (a) {
						b("FBLogger")("login").catching(a).warn("smart lock promise fail")
					}))
			}
		};
		e.exports = a
...
__d("ErrorContextualDialogXUITheme", ["cx"], (function (a, b, c, d, e, f, g) {
		a = "_572t";
		f.wrapperClassName = a;
		b = {
			offset: 12,
			length: 22
		};
...
__d("WebCookieUseBannerController", ["Arbiter", "CSS", "DeferredCookie", "DOM"], (function (a, b, c, d, e, f) {
		"use strict";
		a = {
			init: function (a, c) {
				this.banner = a;
				this.closeButton = c;
				document.body && b("CSS").addClass(document.body, "hasCookieBanner");
				b("DeferredCookie").addDefaultInteractionListener();
				if (c) {
					var d = function () {
						document.body && b("CSS").removeClass(document.body, "hasCookieBanner"),
						b("DOM").remove(a),
						b("Arbiter").inform("WebCookieUseBannerController/close")
					};
					c.addEventListener ? c.addEventListener("click", d) : c.attachEvent && c.attachEvent("onclick", d)
				}
			}
		};
		c = a;
...
__d("BrowserPrefillLogging", ["BanzaiLogger", "DamerauLevenshtein", "ge"], (function (a, b, c, d, e, f) {
		...
		a = {
			initContactpointFieldLogging: function (a) {
				this.contactpointFieldID = a.contactpointFieldID;
				this._updateContactpoint();
				this.serverPrefillContactpoint = a.serverPrefill;
				a = b("ge")(this.contactpointFieldID);
				if (a == null)
					return;
				a.addEventListener("input", this._mayLogContactpointPrefillViaDropdown.bind(this));
				window.addEventListener("load", this._mayLogContactpointPrefillOnload.bind(this));
				return
			},
			registerCallback: function (a) {
				this.regeisteredCallbacks = this.regeisteredCallbacks || [],
				this.regeisteredCallbacks.push(a)
			},
			_invokeCallbacks: function (a, b) {
				if (this.regeisteredCallbacks == null || this.regeisteredCallbacks.size === 0)
					return;
				this.regeisteredCallbacks.forEach(function (c) {
					c(a, b)
				})
			},
			initPasswordFieldLogging: function (a) {
				this.passwordFieldID = a.passwordFieldID;
				this._updatePassword();
				a = b("ge")(this.passwordFieldID);
				if (a == null)
					return;
				a.addEventListener("input", this._mayLogPasswordPrefillViaDropdown.bind(this));
				window.addEventListener("load", this._mayLogPasswordPrefillOnload.bind(this))
			},
			updatePrefill: function (a, c, d) {
				var e,
				f = (e = b("ge"))("prefill_source"),
				g = e("prefill_type"),
			...
			_mayLogContactpointPrefillOnload: function () {
				this._updateContactpoint();
				if (this.previousContactpoint == null || this.previousContactpoint === "")
					return;
				var a = this.previousContactpoint === this.serverPrefillContactpoint ? "server_prefill" : "browser_onload";
				this._logBrowserPrefill(a, "contact_point");
				this._invokeCallbacks(a, "contact_point")
			},
			_mayLogPasswordPrefillOnload: function () {
				this._updatePassword();
				if (this.previousPassword == null || this.previousPassword === "")
					return;
				this._logBrowserPrefill("browser_onload", "password");
				this._invokeCallbacks("browser_onload", "password")
			},
			_mayLogContactpointPrefillViaDropdown: function () {
				var a = b("ge")(this.contactpointFieldID);
				if (a == null || a.value == null)
					return;
				if (this._isBrowserPrefill(this.previousContactpoint, a.value) === !1) {
					this._updateContactpoint();
					return
				}
				this._updateContactpoint();
				this._logBrowserPrefill("browser_dropdown", "contact_point");
				this._invokeCallbacks("browser_dropdown", "contact_point")
			},
			_mayLogPasswordPrefillViaDropdown: function () {
				var a = b("ge")(this.passwordFieldID);
				if (a == null || a.value == null)
					return;
				if (this._isBrowserPrefill(this.previousPassword, a.value) === !1) {
					this._updatePassword();
					return
				}
				this._updatePassword();
				this._logBrowserPrefill("browser_dropdown", "password");
			...
			_isBrowserPrefill: function (a, c) {
				...
				if (c === a)
					return !1;
				if (c.length === 1 || a.length === c.length + 1 || c.length === a.length + 1)
					return !1;
			...
			_updateContactpoint: function () {
				var a = b("ge")(this.contactpointFieldID);
				this.previousContactpoint = a != null && a.value != null ? a.value : ""
			},
			_updatePassword: function () {
				var a = b("ge")(this.passwordFieldID);
				this.previousPassword = a != null && a.value != null ? a.value : ""
			},
			_logBrowserPrefill: function (a, c) {
				var d = null;
				c === "contact_point" && (d = this.previousContactpoint);
				a !== "server_prefill" && this.updatePrefill(d, a, c);
